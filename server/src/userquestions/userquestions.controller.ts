...
export class UserquestionsController {
  ...
  async createQuestion(
    ...
    return await this.userquestionsService.create(
  ...
  async generateDescriptionByAi(@Body() questionData) {
    return await this.userquestionsService.generateDescriptionByAi(
      questionData,
    );
  }
  //
  ...
  async getQuestionTag(
    @Param('tagName01') tagName01,
    @Param('tagName02') tagName02,
  ) {
  ...
  async createComment(@Req() req, @Body() container) {
    ...
    return await this.userquestionsService.createComment(
      commentSchema,
      container.questionId,
    );
  }
  /////////////////////////////////////////////////////////////////////////////////////
  @Post('create-like')
  ...
  async findMyFriendsAllQuestions(
    ...
    @Query('flug') flug: string,
    @Query('tag') tag: string,
    @Req() req,
  ) {
    const id = req.user._id;
    return await this.userquestionsService.findMyFriendsAllQuestions(
      id,
      skip,
      flug,
      tag,
  ...
  async findMyFriendsAllQuestionComments(
    @Query('id') id,
    @Query('page') page: number,
  ) {
    return await this.userquestionsService.findMyFriendsAllQuestionComments(
      id,
      page,
    );
  } //
