...
const EntryPoint = ({user}) => {
    const {myId,fdId,name,profile,title,type,size=26,color='white'} = user
    const handleCallEntry = () => {
        ...
        return (
          <div>
              ...
              {
                type === 'Audio' ?<IoIosCall color={color} size={size}/> : type === "Video" ? <IoVideocam color={color} size={size}/> : null
              }
              </h2>
          </div>
        )
