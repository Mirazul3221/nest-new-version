...
const CommentsContainer = ({ setOpenCommentsBox, question, Handler,handleShare }) => {
  ...
  return (
    <div className="md:w-1/2 md:max-h-8/12 min-h-1/2 rounded-2xl shadow-lg relative bg-white p-2">
      <span
        onClick={() => setOpenCommentsBox(false)}
        className="absolute flex justify-center rounded-full cursor-pointer items-center p-2 bg-gray-100 top-1 right-1"
      >
        <RxCross2 size={20} />
      </span>
      <div className="py-3 border-b text-center font-bold">{`${firstname}'s question`}</div>
      <div className="flex flex-col justify-center">
        <div className="overflow-y-auto h-[50vh] md:h-[60vh]">
          <OnlyCard question={question}  handleShare={handleShare} />
          <div className="px-4">
            {comments?.map((item, i) => {
              return (
                <div ref={commentRef} key={i}>
                  <UserComment item={item} Handler={Handler}/>
                </div>
              );
            })}
            {loading ? (
...
const UserComment = ({ item, Handler }) => {
